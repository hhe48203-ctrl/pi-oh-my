...
function enhanceWithHashes(text: string, startLine: number): string {
	...
	const enhanced = lines.map((line, i) => {
		...
		return `${lineNum}#${lineHash(line)}| ${line}`;
	});
...
function registerHashlineEditTool(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "hashline_edit",
		label: "Hashline Edit",
		description:
		...
		async execute(_toolCallId, params: HashlineEditInput, _signal, _onUpdate, ctx) {
			...
			let parsedEdits: Array<{ start: Anchor; end: Anchor; newContent: string }>;
			try {
			...
			return {
				content: [{ type: "text" as const, text: `Applied ${results.length} edit(s) to ${params.path}:\n${results.map((r, i) => `  ${i + 1}. ${r}`).join("\n")}` }],
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("tool_result", async (event, _ctx) => {
		...
		const enhanced = enhanceWithHashes(textPart.text, startLine);

		const newContent = event.content.map((c) => {
			if (c.type === "text") return { type: "text" as const, text: enhanced };
			return c;
		});
